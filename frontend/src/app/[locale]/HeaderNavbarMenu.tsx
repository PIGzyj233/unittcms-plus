...
import { Button, Link as NextUiLink, ListBoxItem, ListBox } from '@/components/heroui';
...
export default function HeaderNavbarMenu({ messages, locale }: Props) {
  ...
  return (
    <header className="app-chrome sticky top-0 z-40 w-full">
      <nav data-menu-open={isMenuOpen ? 'true' : 'false'}>
        <div className="flex h-16 items-center justify-between gap-4 px-4 lg:px-6">
          <div className="flex min-w-0 items-center gap-5">
            <Link className="flex shrink-0 items-center gap-2" href={getLocalizedPath('/', locale)}>
              <Image src="/favicon/icon-192.png" width={32} height={32} alt="Logo" className="rounded-md" />
              <p className="font-semibold tracking-normal text-inherit">{Config.appName}</p>
            </Link>
              ...
              {commonLinks.map((link) =>
                link.isExternal ? (
                  <NextUiLink
                    ...
                    href={link.href}
                    showAnchorIcon
                    className="rounded-md px-3 py-2 text-sm font-medium text-neutral-600 hover:bg-black/[0.04] dark:text-neutral-300 dark:hover:bg-white/[0.06]"
                  >
                    {link.label}
                  </NextUiLink>
                ) : (
                  <Link
                    key={link.uid}
                    className="rounded-md px-3 py-2 text-sm font-medium text-neutral-700 transition-colors hover:bg-black/[0.04] hover:text-neutral-950 dark:text-neutral-300 dark:hover:bg-white/[0.06] dark:hover:text-white"
          ...
          <div className="flex shrink-0 items-center justify-end gap-2">
            <NextUiLink
              ...
              aria-label="Github"
              className="inline-flex h-9 w-9 items-center justify-center rounded-md text-neutral-500 hover:bg-black/[0.04] hover:text-neutral-950 dark:text-neutral-400 dark:hover:bg-white/[0.06] dark:hover:text-white"
            >
              <GithubIcon />
            </NextUiLink>
            <ThemeSwitch />
        ...
        {isMenuOpen && (
          <div className="border-t border-black/10 bg-white/95 shadow-lg shadow-black/5 dark:border-white/10 dark:bg-neutral-950 md:hidden">
            <div className="mx-4 flex flex-col gap-2 py-3">
